...
#     ax2.set_yscale('log')
    ...
    p2, = ax2.plot(t_ax, C_emi[0].values,label = 'PCHO$_{aer}$',linewidth = 1.5,color = 'b')
    #fill_between_shade(ax2,t_ax, C_omf[0].values,std_omf[0],'b')
    p3, = ax.plot(t_ax, C_atmos[0].values,label= 'SIC',linewidth = 1.5,color = 'b')
    #fill_between_shade(ax,t_ax, C_conc[0].values,std_conc[0],'b')


    print(t_ax, C_emi[0].values)
    p21, = ax2.plot(t_ax, C_emi[1].values,label = 'DCAA$_{aer}$',linewidth = 1.5,color = 'g')
    #fill_between_shade(ax2,t_ax, C_omf[1].values,std_omf[1],'g')
    
    p31, = ax1.plot(t_ax,C_atmos[1],label= 'SST',linewidth = 1.5,color = 'g')
    
    #fill_between_shade(ax,t_ax, C_conc[1].values,std_conc[1],'g')

    p22, = ax3.plot(t_ax, C_emi[2].values,label = 'PL$_{aer}$',linewidth = 1.5,color = 'darkred')
    p23, = ax4.plot(t_ax, C_emi[3].values,label = 'SS$_{aer}$',linewidth = 1.5,color = 'm')
    ...
    ax.set_ylabel("SIC", fontsize = font)
#    ax.set_ylim(limits[0])
    ax.yaxis.set_tick_params(labelsize=font)

    ax2.set_ylabel("Emission flux",fontsize = font)
#    ax2.set_ylim(0, 0.015)
    ax3.set_ylabel("Emission flux PL$_{aer}$", color='darkred', fontsize = font)
#    ax3.set_ylim(0,0.5)
...
def plot_all_seasonality(dict_seasonality):
    fig, axs = plt.subplots(1,2,figsize = (12,6)) #15,8
    ax = axs.flatten()
 
    factor = 1e12
    emi_pol_mean = get_mean(dict_seasonality['emi']['seasonality']['emi_POL'])*factor
    emi_pro_mean = get_mean(dict_seasonality['emi']['seasonality']['emi_PRO'])*factor
    emi_lip_mean = get_mean(dict_seasonality['emi']['seasonality']['emi_LIP'])*factor
    ...
    plot_monthly_series_pannel(ax, fig,
            ...
            left_axis=False)
    fig.tight_layout()
    plt.savefig(f'Multiannual_monthly_emission_trends_poles.png',dpi = 300, bbox_inches="tight")
...
#     p1 = ax.plot(t_ax, C_conc[1],color = c,linestyle = 'dashed',label = 'PL',linewidth = lw)
    ...
    ax.set_ylabel(ylabels[0], fontsize = font)
    #ax.set_ylim(0, ylims[0])
    ax.yaxis.set_tick_params(labelsize=font)

    ax2.set_ylabel(ylabels[1], fontsize = font)
...
def get_mean_reg(data_ds):
    ...
    for idx, reg_na in enumerate(list(reg_data.keys())):
        ...
        reg_data[reg_na] = reg_sel_vals.mean(dim=['lat', 'lon'], skipna=True)
    return reg_data

def plot_seasonality_region(dict_seasonality):
    emi_means = [get_mean_reg(dict_seasonality['emi']['seasonality']['emi_POL']*factor),
            get_mean_reg(dict_seasonality['emi']['seasonality']['emi_PRO']*factor),
            get_mean_reg(dict_seasonality['emi']['seasonality']['emi_LIP']*factor)]
    print('finished computing means emi')
    ...
    for i in range(len(emi_means)):
        plot_seanonality_reg_species(seaice_mean, emi_means[i], var_id[i], lims)
...
def plot_seanonality_reg_species(C_ice, C_emi, var_id, lims):
    ...
    leg_list = []
    color_reg = ['k','r','m','pink','lightgreen','darkblue','orange',
    ...
    for idx,na in enumerate(C_emi.keys()):
        ...
        p2, p3 = plot_seasons_reg(axs,C_emi,C_ice,\
                                na, color_reg[idx],lw, \
                                ylabels, lims)
        leg_list.append(p2)
    ...
    for i,axs in enumerate(ax):
        axs.set_title(titles_1[i], loc='right', fontsize = font)
    ...
    plt.savefig(f'Multiannual monthly trends poles and subregions_color_reg_ice_{var_id}.png',dpi = 300, bbox_inches="tight")
